...
날고긴다는 사람들 다 모아서 만든 우주선마저 소프트웨어 결함으로 문제가 생기는걸 보면 암만 천재들만 모아놔도 알아서 안정성이 확보되는건 아닌가봅니다.
...
<p class="quotes">
<i class="ri-double-quotes-l quote"></i>
테스트는 쉽게 말해 “cancelPayment 함수를 실행한 뒤에는 항상 Payment가 canceled상태로 바뀌어 있어야 해” 라는 조건을 강제하는 것과 같습니다. cancelPayment 내부에서 무슨 일이 일어나든 함수가 정상적으로 수행된 뒤에 Payment는 canceled 상태여야 하는 것이죠.
<i class="ri-double-quotes-r quote"></i>
</p>
