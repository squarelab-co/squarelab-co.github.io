...
`Gerrit`과 `Git` CLI를 사용할 예정이며, 널리 쓰이고 있는 서비스인 `GitHub`에서의 과정과도 비교해 보겠습니다. 이후 과정은 `Git`, `Gerrit`, `GitHub` 등에 대한 기본적인 지식은 갖추고 있고, 간단한 사용법에는 익숙한 사용자를 대상으로 하고 있습니다.

## 들어가기

기존의 Repository에 새로운 파일들을 추가하고 필요한 내용을 채워넣는 작업들을 진행하고 이에 대해 리뷰를 요청한 상황을 가정해 보겠습니다. `Gerrit`의 Web UI에는 다음과 같이 표시되었습니다.
...
위의 "들어가기"에서 설정한 것과 동일한 상태의 `GitHub` Web UI 구성입니다. 5개의 커밋이 하나의 큰 작업을 이루고 있고, 이 변경 사항에 대해 동일한 리뷰 의견을 받았습니다.

![GitHub 리뷰 요청 상태](/images/blog/git-gerrit-review-demo/github-review-01.png)

이 화면은 하나의 `Pull request`를 종합하여 표시해주고 있는데, `Gerrit`과의 가장 큰 차이가 바로 이 `PR`의 유무입니다. `GitHub`은 리뷰 관리를 위한 자체적인 단위로 이 `PR`이라는 개념을 사용하고 있는데, 의견 교환 및 코드 검토, 그리고 수정 및 최종 머지 역시 `PR`을 기준으로 처리됩니다.
...
2. 대상 커밋들을 수정할 준비
    ...
    - 아래와 같은 명령어를 통해 리베이스 과정을 시작합니다. 이 과정에서 수정할 커밋을 지정할 수 있습니다.
        ```bash
    ...
    - 이것을 아래와 같이 변경하고 저장합니다. 리베이스 과정에서 편집할 커밋들을 골라 `edit`로 마킹합니다.
        ```
...
7. 리베이스 과정을 진행시킵니다. 수정하기로 한 두 개의 커밋을 모두 수정 완료한 이후이므로 리베이스 작업이 완료된 상태가 됩니다.
    ```bash
    $ git rebase --continue
    ```

8. `Gerrit`에 다시 리뷰 요청을 해서 변경사항이 반영되도록 합니다.
    ```bash
    $ git review
    ```
    ```
...
`Gerrit`과 `GitHub` 각각의 도구를 사용한 경우를 비교해보면, 비슷한 듯 하면서도 차이점이 있습니다.
...
그러나 계속 덧붙여지는 커밋과 `PR`로 인해 보기에 따라서는 너무 장황해지는 부분이 존재하고, 호불호가 갈리기도 합니다. (`PR`의 머지 정책에 따라 어느 정도 조절할 수 있긴 하지만, 수정 과정 자체는 거의 동일하고, Trade-off가 생깁니다.)
그리고 효과적인 사용을 위해서는 (`Git`이 아닌) `GitHub` 자체의 기능을 많이 배워야 하는 부분이 있습니다.

`Gerrit`은, `Git`을 보조해주는 도구로서의 성격이 보다 강합니다. 리뷰를 위해 공유되는 내용의 형식이나 분량이 조금 더 간결하고 가벼운 편입니다.
